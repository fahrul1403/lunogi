...
const ContentSection = ({ 
  ...
  return (
    <div>
      {item.title && <h3 className="text-2xl font-semibold mb-3">{item.title}</h3>}
      {renderProductFeature && price && imageSrc && productLink ? (
        ...
        item.description && (
          <p className="bg-blue-100 text-xl p-5 rounded-xl mb-4">{item.description}</p>
        )
      )}
      {item.sections.map((section, sectionIndex) => (
        <div key={sectionIndex} className="">
          {section.type === 'table' ? (
            ...
            {section.title && !section.h3image &&
            <h4 className="mb-3 mt-10 text-xl font-medium">{section.title}</h4>
            }
            <div className="flex flex-col md:flex-row items-start gap-4">
              ...
              <div className="flex-1">
                {(section.items as Array<string>).length === 1 ? (
                  <div>
                    {section.h3image && (<h4 className="mb-3 mt-10 text-xl font-medium">{section.title}</h4>)}
                    <p className="mb-5">{(section.items as Array<string>)[0]}</p> 
                  </div>
                ) : (
    ...
    return (
        ...
        <motion.div className="child" variants={childrenVariant}>
            ...
            <div className="relative z-10 flex flex-col space-y-6 items-start w-full px-7 lg:px-[156px] lg:py-[160px]">
                ...
                <h1 className="text-white font-semibold text-3xl lg:text-4xl w-[550px]">sample page<br></br>to help you create the blog</h1>
