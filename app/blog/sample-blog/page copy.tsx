...
type TableContent = {
  ...
  columnWidths?: string[]; // This will now accept fractions like '1/2', '1/3', etc.
  showXBorders?: boolean; // New property
  showYBorders?: boolean; // New property
};
...
type GridContent = {
  ...
  items: {
    ...
    description: string;
    image?: ImageContent;
    textAlign?: 'left' | 'center' | 'right'; // New property
  }[];
  columns: number;
};
...
const article = {
    ...
    references: [
    {
        title: "Views of Indonesian consumers towards medical tourism experience in Malaysia",
        author: "Saragih, H. S., & Jonathan, P.",
        year: "2019"
    },
    {
        title: "Medical tourism in Malaysia: International movement of healthcare consumers and the commodification of healthcare",
        author: "Chee, H. L.",
        year: "2008"
    }
    ]
...
const renderSection = (section: { 
    ...
    text: ContentItem[];
    subsections?: any[];
    align?: 'left' | 'center' | 'right'; // Alignment property for the header
}, level = 1) => {
    ...
    // Alignment class only for the header
    ...
    return (
        <div key={`section-${section.content}`}>
            ...
            const textAlign = item.textAlign || 'left'; // Default to left alignment for text items
            switch (item.type) {
                    ...
                    case 'table':
                        ...
                        return (
                            <div key={`table-${index}`} className={contentMargin} style={{ backgroundColor: bgColor }}>
                                {tableContent.title && (
                                    <h3 className="text-xl font-semibold mb-4">{tableContent.title}</h3>
                                )}
                        ...
                        case 'grid':
                            ...
                            return (
                                <div key={`grid-${index}`} className={`${contentMargin}`} style={{ backgroundColor: bgColor }}>
                                    {gridContent.title && (
                                        <h3 className="text-xl font-semibold mb-4">{gridContent.title}</h3>
                                    )}
                        ...
                        <div key={`product-${index}`} className={`my-8 p-6 border rounded-lg shadow-md ${contentMargin}`} style={{ backgroundColor: bgColor }}>
                            <div className="flex flex-col md:flex-row">
                                ...
                                <div className="md:w-2/3">
                                    <h3 className="text-xl font-bold mb-2">{productContent.name}</h3>
